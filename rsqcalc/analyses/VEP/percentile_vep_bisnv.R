...
optionList <- list(
  ...
  make_option(c("-v", "--vep"), type="character", help="File with VEP annotations"),
  make_option(c("-o", "--out"), type="character", help="Output filename")
)

parser <- OptionParser(
  usage="%prog -r results -v vep -o out",
...
vep=fread(opt$vep, header=FALSE)
vep=unique(vep)
colnames(vep)=c("CHR","POS","ALT","IMPACT")
results=merge(results, vep, by=c("CHR","POS","ALT"), all.x=TRUE)
...
#Aggregate
resultsag=aggregate(results[,c("Core_1000G", "Core_HRC", "Core_TOPMed", "OmniExp_1000G","OmniExp_HRC", "OmniExp_TOPMed", "Omni25_1000G", "Omni25_HRC", "Omni25_TOPMed","MEGA_1000G", "MEGA_HRC", "MEGA_TOPMed")],by=list(results$bin, results$IMPACT), FUN=percent)
print("Percentiles calculated")
resultsag$MAF=aggregate(results[,c("MAF")], by=list(results$bin, results$IMPACT), FUN=mean, na.rm=TRUE)$MAF
