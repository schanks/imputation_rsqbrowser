...
optionList <- list(
  ...
  make_option(c("-s", "--sv"), type="character", help="Structural variants file"),
...
parser <- OptionParser(
  usage="%prog -r results -s sv -o output",
...
svs = fread(opt$sv, fill=TRUE)
print("Structural variants file read")
colnames(svs)=c("START","END","TYPE")
svs$START=as.numeric(svs$START)
svs$END=as.numeric(svs$END)

results$SV=rep(0,length(results$POS))
for (i in 1:length(results$POS)){
	sv = svs[which((svs$START>results$START[i] & svs$START<results$END[i]) | svs$END> results$START[i] & svs$END<results$END[i]),]
	results$SV[i]=nrow(sv)
	if (i %% 1000==0) {
		Sys.sleep(0.01)
