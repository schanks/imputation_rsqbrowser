...
ggplot(all, aes(x=MAF, y=OmniExp_TOPMed, color=IMPACT))+scale_x_log10(breaks=brks, labels=labs, limits=c(1e-4,0.5),expand=c(0,0))+scale_y_continuous(breaks=ybrks,limits=c(0,1), expand=c(0,0))+theme_bw()+theme(legend.position="bottom",panel.grid.major=element_line(colour="#e7e7e7"),panel.spacing=unit(0.5, "lines"),axis.title=element_text(size=8),axis.text=element_text(size=7),panel.border=element_rect(color="black"), legend.key.width=unit(3, "lines"),legend.text=element_text(size=7),legend.title=element_text(size=8), strip.background=element_rect(fill="white", color=NA), strip.text=element_text(size=8))+geom_line(aes(group=IMPACT), size=0.6)+facet_grid(.~Ancestry)+scale_color_manual(values=c("#1b9e77", "#e6ab02","#7570b3","#e7298a"))+xlab("WGS Minor Allele Frequency")+ylab(expression(atop(Proportion~of~TOPMed~SNVs,well-imputed~(r^2>0.8))))+guides(color=guide_legend(nrow=2))
dev.off()
