...
ggplot(all, aes(x=MAF,y=value, color=Type))+scale_x_log10(breaks=brks, labels=labs, limits=c(1e-4,0.5),expand=c(0,0))+scale_y_continuous(breaks=ybrks,limits=c(0,1), expand=c(0,0))+theme_bw()+facet_grid(Array~Ancestry)+theme(legend.position="bottom",legend.text=element_text(size=7),panel.grid.major=element_line(colour="#e7e7e7"),panel.spacing=unit(0.8, "lines"), strip.text=element_text(size=8),axis.title=element_text(size=8),axis.text=element_text(size=7),panel.border=element_rect(color="black"),strip.background=element_rect(fill="white",color=NA), legend.title=element_text(size=8), plot.margin=unit(c(0.5,0.5,0.5,0.5), "lines"),legend.key.width=unit(3,"lines"))+geom_line(aes(group=Type), size=0.7)+xlab("WGS Minor Allele Frequency")+ylab(expression(atop(Proportion~of,well-imputed~variants)))+guides(color=guide_legend(nrow=2))+scale_color_manual(values=c("#1b9e77", "#e6ab02","#7570b3","#e7298a"))
dev.off()
