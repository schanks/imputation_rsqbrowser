...
ggplot(all, aes(y=yval))+geom_vline(xintercept=0)+geom_point(aes(x=Beta, col=Ancestry))+geom_segment(aes(y=yval, yend=yval, x=Beta-1.96*SE, xend=Beta+1.96*SE, col=Ancestry))+facet_grid(Parameter~., scales="free")+scale_y_continuous(breaks=ybrks, labels=ylabs)+theme_bw()+scale_color_manual(values=c("#e41a1c","#4daf4a","#377eb8","#984ea3"))+theme(legend.text=element_text(size=7), legend.title=element_text(size=8), strip.text=element_text(size=8), axis.title=element_text(size=8), axis.text=element_text(size=7),panel.grid.minor=element_blank(),strip.background=element_rect(fill="white",color=NA))+ylab("Predictor")
dev.off()
