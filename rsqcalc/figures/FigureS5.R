...
a=ggplot(all, aes(y=Concordance, x=Ancestry, fill=Ancestry))+geom_violin(aes(color=Ancestry))+geom_boxplot(coef=0,lwd=0.2, outlier.shape=NA, alpha=0)+scale_y_continuous(limits=c(0.3,1))+scale_color_manual(values=c("#cb181d","#238b45","#2171b5","#6a51a3"))+scale_fill_manual(values=c("#cb181d","#238b45","#2171b5","#6a51a3"))+ylab("Concordance")+theme_bw()+ggtitle("Concordance")+theme(plot.margin=unit(c(0.5,0.5,0.5,0.5), "lines"),plot.title=element_text(color="white",vjust=-55),legend.position="none",axis.title.x=element_text(color="white", size=8),axis.title.y=element_text(size=8), axis.text=element_text(size=6))+guides(color=guide_legend(nrow=2))
...
tiff(file="~/FigS5.tiff", height=174, width=174, units="mm", res=300)
